@if ($started && $sale)
    @php
        ...
        $discountTotal = $sale->items->sum('discount');
        ...
        <div class="pdv-wrap mt-6">
            ...
            <div class="pdv-top">
                ...
                <div class="pdv-kpis min-w-0">
                    ...
                    <div class="pdv-kpi">
                        ...
                        <div class="val">R$ {{ number_format($sale->total, 2, ',', '.') }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="pdv-grid min-w-0">
                ...
                <div class="space-y-4 min-w-0">
                    ...
                    {{-- Adicionar item --}}
                    <div class="pdv-panel pdv-searchbox min-w-0">
                        <div class="pdv-panel-head">
                            <div class="pdv-panel-title">Adicionar item</div>
                            <div class="pdv-hint">Busque e confirme o produto</div>
                        </div>

                        {{ $this->saleForm }}

                        @php
                            $searchTerm = trim((string) ($this->saleData['product_search'] ?? ''));
                        @endphp

                        @if (! empty($this->productSearchResults))
                            <div class="pdv-results">
                                @foreach ($this->productSearchResults as $result)
                                    <button
                                        type="button"
                                        class="pdv-result"
                                        wire:click="selectProduct({{ $result['id'] }})"
                                    >
                                        <div class="pdv-result-info">
                                            <div class="pdv-result-name">
                                                {{ $result['name'] }}
                                            </div>
                                            <div class="pdv-result-meta">
                                                {{ $result['barcode'] ? 'Cód: ' . $result['barcode'] : 'Sem código' }}
